...
public class CurrentLocationRestarterBroadcastReceiver extends BroadcastReceiver {
    public static final String TAG = CurrentLocationRestarterBroadcastReceiver.class.getSimpleName();
    private static JobScheduler jobScheduler;
    private CurrentLocationRestarterBroadcastReceiver restartLocationServiceReceiver;
    ...
    private void registerRestarterReceiver(final Context context) {
        ...
        if (restartLocationServiceReceiver == null)
            restartLocationServiceReceiver = new CurrentLocationRestarterBroadcastReceiver();
        else try{
            context.unregisterReceiver(restartLocationServiceReceiver);
        } catch (Exception e){
            // not registered
        }
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    context.registerReceiver(restartLocationServiceReceiver, filter);
                } catch (Exception e) {
                    try {
                        context.getApplicationContext().registerReceiver(restartLocationServiceReceiver, filter);
                    } catch (Exception ex) {
